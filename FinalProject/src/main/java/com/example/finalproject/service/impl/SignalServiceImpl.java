...
public class SignalServiceImpl implements SignalService {
    ...
    public List<String> distinctSymbols() {
        return mongoTemplate.query(SignalRecord.class)
                ...
                .stream()
                .filter(s -> s != null && !s.isBlank()) // null veya boş string'leri filtrele
    ...
    public List<SummaryBucket> summary(SummaryRequest req) {
        ...
        switch (groupBy) {
            ...
            default -> { // day
                project = project()
                        .andExpression("{$dateToString: {format: '%Y-%m-%d', date: '$ts', timezone: 'UTC'}}")
                        .as("bucket")
                        .andInclude("signal");
            }
        }
        ...
        AggregationResults<org.bson.Document> res = mongoTemplate.aggregate(agg, "signals", org.bson.Document.class);

        // Map to DTO
        return res.getMappedResults().stream().map(d -> SummaryBucket.builder()
